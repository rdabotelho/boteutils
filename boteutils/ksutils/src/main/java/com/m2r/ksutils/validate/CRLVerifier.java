...
public class CRLVerifier {
		...
		try {
			...
			for (String crlDP : crlDistPoints) {
				...
				if (crl.isRevoked(cert)) {
					throw new CertificateVerificationException(ValidationErro.REVOKED, crlDP);
				}
			}
		} catch (Exception ex) {
			if (ex instanceof CertificateVerificationException) {
				...
				throw new CertificateVerificationException(ValidationErro.CRL_CONNECTION, cert.getSubjectX500Principal().getName() + "\nErro: "+ ex.getMessage(), ex);
			}
		}
	}
	...
	private static X509CRL downloadCRL(String crlURL)
		...
		if (crlURL.startsWith("http://") || crlURL.startsWith("https://") || crlURL.startsWith("ftp://")) {
			...
			throw new CertificateVerificationException(ValidationErro.OTHER, "Can not download CRL from certificate " + "distribution point: " + crlURL);
		}
	}
