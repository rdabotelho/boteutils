...
public class CertificateVerifier {
	...
	public static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert, Set<X509Certificate> additionalCerts, boolean validateCRL)
					throws CertificateVerificationException {

		try {
			// Check validity period
			LocalDate now = LocalDate.now();
			LocalDate begin = LocalDateTime.ofInstant(cert.getNotBefore().toInstant(), ZoneId.systemDefault()).toLocalDate();
			LocalDate end = LocalDateTime.ofInstant(cert.getNotAfter().toInstant(), ZoneId.systemDefault()).toLocalDate();
			if (now.isBefore(begin) || now.isAfter(end)) {
				throw new CertificateVerificationException(ValidationErro.VALIDITY_PERIOD);
			}

			// Check for self-signed certificate
			if (isSelfSigned(cert)) {
				throw new CertificateVerificationException(ValidationErro.SELF_SIGNED);
			}
			...
			throw new CertificateVerificationException(ValidationErro.BUILDING, certPathEx.getMessage(), certPathEx);
		}
		catch (CertificateVerificationException cvex) {
			throw cvex;
		}
		catch (Exception ex) {
			throw new CertificateVerificationException(ValidationErro.OTHER, ex.getMessage(), ex);
		}
	}
