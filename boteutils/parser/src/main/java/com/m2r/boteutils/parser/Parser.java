...
public abstract class Parser<R> {
	...
	protected Token nextToken(String sequence) throws ParserException {
		...
		if (subSequence.length() > 0) {
			throw new LexicalException("Token não identificado na posição " + (pos+1), pos+1);
		}
		return null;
	}
	...
	static protected class Token {
		...
		public Token(ITokenType type, String value, int pos) {
			this.type = type;
			this.value = value;
			this.pos = pos;
		}
	...
	protected void syn() throws ParserException {
		pos = 0;
		if (!(exp() && isEnd())) {
			throw new SyntacticException("Estrutura da expressão incorreta", pos+1);
		}
	}
